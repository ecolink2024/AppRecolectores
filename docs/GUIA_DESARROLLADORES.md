...
**Cambios recientes (jun 2026):** **Preparación de insumos**; tablas operario con scroll; **Ver detalle** de ruta/parada; **Obs. recolector** y **cestos**; tipo **Punto**; Maps con GPS; firmas en Storage; WhatsApp **Avisar**; cierre operario; **dos montos** en rutas/KPIs (`totalPrecio` vs `recaudado`); gráfico mensual dual; serie KPI 48 meses (`operario-kpis-serie-mensual.ts`); migraciones en `supabase/apply-pending-operativo.sql`.
...
- `src/lib/data/operario-kpis.ts` — KPIs del período filtrado: rutas por `fecha` en rango, `.limit(5000)` + recolecciones
...
- Agregación del período: `buildOperarioKpis()` sobre rutas del rango (`rutas.fecha`) y sus `ruta_recolecciones`
- Serie mensual: `fetchSerieMensualRecaudacion()` — **48 meses**, agrupado por mes de `rutas.fecha`; **no** usa el filtro Desde/Hasta del dashboard
...
- **Rutas realizadas:** cuenta `completada` **y** `cerrada` (antes solo `completada`)
