...
- Agregación: `buildOperarioKpis()` — conteos de estado sobre todo el historial; **montos, materiales, zonas, recolectores, gastos, km** solo con `rutaImpactaKpis` / `RUTA_ESTADOS_KPI_IMPACTO` (`cerrada`). Una `completada` cuenta en “Pendiente cierre” pero no mueve recaudación hasta el cierre operario
...
- **Rutas del período (conteos):** estados historial (`completada`, `cerrada`, `cancelada`)
- **Impacto numérico (montos/materiales/serie):** solo `cerrada` (`rutaImpactaKpis`)
- **Pendiente cierre:** cuenta `completada` (`rutas.pendientesCierre`; CSV: «Pendiente cierre operario») — no mueve montos
- **Rutas realizadas:** cuenta `completada` **y** `cerrada` (conteo; ingresos solo de cerradas)
- **Ingresadas (recolecciones):** paradas de rutas **cerradas** (cualquier estado de parada)
